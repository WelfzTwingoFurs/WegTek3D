...
#		sprites[angles].scale.x = OS.window_size.x*2
	...
	if window_size_check != OS.window_size:
		...
		var get_edgy = abs(tan(deg_rad(rays[angles-1].rotation_degrees)) - tan(deg_rad(rays[angles-1].rotation_degrees)))                  #Distance between border renders
		...
		var render_size = tan(deg_rad(rays[0].rotation_degrees) + get_edgy) - (tan(deg_rad(rays[angles-1].rotation_degrees)) - get_edgy) #Distance between screen edges
		#this re-uses the XKUSU code and must be way less efficient, but otherwise won't work?
		
		#var rendering_size = ( (position + (Vector2(0,draw_distance).rotated(rotation_angle))) - position ).angle()
		#var rendering_size = -abs(((( position + (Vector2(0,draw_distance).rotated(rotation_angle + rays[0].rotation_degrees)) ) - position).angle()) - ((( position + (Vector2(0,draw_distance).rotated(rotation_angle + rays[angles-1].rotation_degrees)) ) - position).angle())) *0.8
		#var rendering_size = -abs(((( position + (Vector2(0,draw_distance).rotated(rotation_angle + rays[0].rotation_degrees)) ) - position).angle()) - ((( position + (Vector2(0,draw_distance).rotated(rotation_angle + rays[angles-1].rotation_degrees)) ) - position).angle())) *0.8
		
		#var rendering_size = ( tan(Vector2(0,draw_distance).rotated(rotation_angle + rays[0].rotation_degrees).angle()) - tan(Vector2(0,draw_distance).rotated(rotation_angle + rays[angles-1].rotation_degrees).angle()) )
		#var rendering_size = ( tan(((position + Vector2(0,draw_distance).rotated(rotation_angle + rays[0].rotation_degrees)) - position).angle()) - tan(((position + Vector2(0,draw_distance).rotated(rotation_angle + rays[angles-1].rotation_degrees)) - position).angle()) )
		
		#var rendering_size = -abs(  (position + (Vector2(0,draw_distance).rotated(rotation_angle + rays[0].rotation_degrees)) - position).angle()  +  (position + (Vector2(0,draw_distance).rotated(rotation_angle + rays[angles-1].rotation_degrees)) - position).angle()  )
		#var rendering_size = tan(deg_rad(rays[0].rotation_degrees)) - (tan(deg_rad(rays[angles-1].rotation_degrees))) #Distance between screen edges
		var rendering_size = rays[0].rotation_degrees - rays[angles-1].rotation_degrees #Distance between screen edges
		
		#$PolyContainer.scale.x = 0.0005 * abs(OS.window_size.x/render_size)
		#$PolyContainer.scale.x = -render_size
		#$PolyContainer.scale.x = rendering_size
		

		#$PolyContainer.scale.x = abs(OS.window_size.x/(angles/angles_mult))
...
onready var highlighted_map_ray = (angles*angles_mult)/2
#onready!!
	...
	for n in angles-1: #fix this crap will ya? about time we start rendering with one less, it creates a gap when strafing left
		...
		if rays[n].is_colliding():
			...
			if (obj.is_in_group("wall")) && obj != wall_rendering_now:
				...
				var xkusu1 = OS.window_size.x * holyshit
				#var xkusu1 = (OS.window_size.x * (0.5 * tan(holyshit)))
				...
				#var lineH1 = (OS.window_size.y / distance1)   /  cos(holyshit+midscreen)
				...
				var xkusu2 = OS.window_size.x * holyshit
				#var xkusu2 = (OS.window_size.x * (0.5 * tan(holyshit)))
				...
				#var lineH2 = (OS.window_size.y / distance2)   /  cos(holyshit+midscreen)
