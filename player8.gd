...
#	print(texture_width," -  ",(texture_width/100)*100 ,"      ",texture_cellsize)
	...
	if draw_dist_type == 2:
		#draw_type2_minH = (OS.window_size.y / (sqrt(pow(0, 2) + pow((draw_distance), 2)) )) /  cos( deg_rad(0) )
...
export var draw_dist_type = 2
export var draw_type1_fade = 1.5
...
func _draw():
	...
	for n in angles-1:
		if rays[n].is_colliding():
			...
			elif draw_dist_type == 1:
				var Cfade 
				if distance > draw_distance/draw_type1_fade:
					#Cfade = 255/distance#(draw_distance-(draw_distance/1.5))
					#Cfade = 255/(distance-draw_distance/1.5)
					Cfade = 255/(distance-(draw_distance/draw_type1_fade))
					#print(distance)
				else:
					Cfade = 255
				#print(distance)
			...
			elif draw_dist_type == 2:
				#draw_type2_minH = (OS.window_size.y / (sqrt(pow(0, 2) + pow((draw_distance), 2)) )) /  cos( deg_rad(0) )
				
				#if distance > draw_distance/draw_type1_fade:
					#Cfade = -brightMax /(distance-(draw_distance/draw_type1_fade))
					#Cfade = -(draw_type2_minH/brightness) /(distance-(draw_distance/draw_type1_fade))
					#Cfade = -(draw_type2_minH/brightness) /(distance)
				var Cfade = -((OS.window_size.y/draw_type2_minH)*brightness) /(distance-draw_distance)
					#Cfade = -(draw_type2_minH*brightMax) /(distance-draw_distance)
				#else:
					#Cfade = -brightMax
					#Cfade = 0#
...
#				
#				sprites[n].position.x = (OS.window_size.x * (0.5 * tan(deg_rad(rays[n  ].rotation_degrees))   ))# / tan((0.5 * (angles*angles_divi)))
#				
#				var xkusu_next = (OS.window_size.x * (0.5 * tan(deg_rad(rays[n+1].rotation_degrees))   ))# / tan((0.5 * (angles*angles_divi)))
#				var xkusu      = (OS.window_size.x * (0.5 * tan(deg_rad(rays[n  ].rotation_degrees))   ))# / tan((0.5 * (angles*angles_divi)))
#				sprites[n].scale.x = (xkusu - xkusu_next - angles_divi/10)-line_gap_compensate
